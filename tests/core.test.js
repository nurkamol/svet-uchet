...
// --- три эпохи тарифа: плоский (авг 2019 – май 2024), блочный старый, блочный новый
eq('до мая 2024 — плоский 295',   prices(2023, 0, 'regular'), [295,295,295,295,295,295]);
eq('апрель 2024 — ещё плоский',   prices(2024, 3, 'regular'), [295,295,295,295,295,295]);
eq('май 2024 — блочный старый',   prices(2024, 4, 'regular'), [600,800,1000,1500,1750,2000]);
eq('плоский для электроплит — половина', prices(2020, 5, 'stove'), [147.5,147.5,147.5,147.5,147.5,147.5]);
// плоский тариф = кВт·ч × цена: все блоки по одной цене дают линейную оплату
...
eq('плоский электроплиты вдвое дешевле',
   api.blockCost(300, api.tariffFor(2023,0,'stove')) * 2, api.blockCost(300, api.tariffFor(2023,0,'regular')));
// для сравнения — насколько блочный набор завышал старый месяц
eq('старый месяц по блочному был бы вдвое дороже',
   api.blockCost(300, api.toTariff([600,800,1000,1500,1750,2000])) > api.blockCost(300, api.tariffFor(2023,0,'regular'))*2, true);
