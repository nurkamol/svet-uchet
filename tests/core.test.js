...
// --- четыре эпохи тарифа
eq('с 1 июня 2026 — набор 650', prices(2026, 6, 'regular'), [650,900,1100,1600,1900,2200]);
eq('с 1 мая 2025 — набор 600',  prices(2025, 5, 'regular'), [600,800,1000,1500,1750,2000]);
eq('апрель 2025 — ещё набор 450', prices(2025, 3, 'regular'), [450,900,900,1350,1575,1800]);
eq('с 1 мая 2024 — набор 450',  prices(2024, 4, 'regular'), [450,900,900,1350,1575,1800]);
eq('апрель 2024 — плоский 295', prices(2024, 3, 'regular'), [295,295,295,295,295,295]);
eq('2023 — плоский 295',        prices(2023, 0, 'regular'), [295,295,295,295,295,295]);
// границы каждой смены — с 1-го числа
eq('май 2025 vs апрель 2025 — разные наборы',
   [prices(2025,4,'regular')[0], prices(2025,3,'regular')[0]], [600, 450]);
eq('июнь 2026 vs май 2026 — разные наборы',
   [prices(2026,5,'regular')[0], prices(2026,4,'regular')[0]], [650, 600]);

// эпоха 05.2024–04.2025: слитый блок 201–1000 @900 = два блока по 900 (равно реальному 5-блочному)
const real2024 = api.blockCost(800, api.toTariff([450,900,900,1350,1575,1800]));
eq('слитый блок: 800 кВт·ч = 200×450 + 600×900', real2024, 200*450 + 600*900);
eq('слитый блок: 300 кВт·ч = 200×450 + 100×900', api.blockCost(300, api.tariffFor(2024,7,'regular')), 200*450 + 100*900);

// плоский тариф = кВт·ч × цена
eq('плоский: 300 кВт·ч × 295', api.blockCost(300, api.tariffFor(2023,0,'regular')), 88500);
eq('плоский: 1500 кВт·ч × 295 (без роста по блокам)', api.blockCost(1500, api.tariffFor(2023,0,'regular')), 1500*295);

// электроплиты — ровно половина во всех четырёх эпохах
for (const [y,m,эпоха] of [[2026,6,'2026'],[2025,5,'2025'],[2024,7,'2024'],[2023,0,'плоский']]){
  eq('электроплиты вдвое дешевле, '+эпоха,
     api.blockCost(800, api.tariffFor(y,m,'stove')) * 2, api.blockCost(800, api.tariffFor(y,m,'regular')));
}
eq('плоский электроплиты — 147,5', prices(2020, 5, 'stove'), [147.5,147.5,147.5,147.5,147.5,147.5]);
eq('2024 электроплиты — 225/450/…', prices(2024, 7, 'stove'), [225,450,450,675,787.5,900]);

// --- граница достоверности сдвинулась на август 2019 (появился плоский тариф)
const mo2 = (y, m) => ({y, m});
eq('известная дата — 1 августа 2019', api.TARIFF_KNOWN_FROM, '2019-08-01');
eq('июль 2019 — старее известного', api.monthsBeforeKnownTariff([mo2(2019,6)]).length, 1);
eq('август 2019 — уже известен',    api.monthsBeforeKnownTariff([mo2(2019,7)]).length, 0);
eq('2023 — известен (плоский тариф)', api.monthsBeforeKnownTariff([mo2(2023,0)]).length, 0);
eq('2026 — известен', api.monthsBeforeKnownTariff([mo2(2026,0)]).length, 0);
eq('фильтрует только доисторические',
   api.monthsBeforeKnownTariff([mo2(2018,0), mo2(2019,6), mo2(2019,7), mo2(2023,0)]).map(x=>[x.y,x.m]),
   [[2018,0],[2019,6]]);
